...
export async function getMyPuzzles(filters = {}) {
  ...
  const response = await fetch(`/api/puzzles/user/my?${params.toString()}`, {
    headers: getAuthHeaders()
  })
...
export async function getLeaderboard(puzzleId) {
  const response = await fetch(`/api/scores/puzzle/${puzzleId}`, {
    headers: getAuthHeaders()
  })
...
export async function getUserScores(puzzleId) {
  const response = await fetch(`/api/scores/puzzle/${puzzleId}/user`, {
    headers: getAuthHeaders()
  })
